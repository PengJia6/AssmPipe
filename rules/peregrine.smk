peregrine = "/data/home/pengjia/mybin/peregrine"
...
peregrine_env_python = "/data/home/pengjia/miniconda3/envs/peregrine/bin"
peregrine_src = "/data/home/pengjia/mysoftware/assm/Peregrine/Peregrine-pg1.6.3/py/scripts/pg_run.py"

rule all:
    input:
        expand(dir_assm + "02_peregrine_{para}/{assm}.{hap}.peregrine_{para}/{assm}.{hap}.peregrine_{para}.ok",para=[
            "default"],
...
# expand(   dir_assm + "}",
#     hap=["hap1", "hap2"]),
# expand(dir_assm + "ChineseQuartet/peregrine_r4/{hap}/ChineseQuartet.HiFi.peregrine_r4.{hap}", hap=["hap1", "hap2"])
...
rule peregrine:
    input:
        fqs=get_fqs
    output:
        dir_assm + "02_peregrine_{para}/{assm}.{hap}.peregrine_{para}/{assm}.{hap}.peregrine_{para}.ok"
    threads: 90
    log:
        dir_assm + "logs/02_peregrine_{para}/{assm}.{hap}.peregrine_{para}.log"
    benchmark:
        dir_assm + "logs/02_peregrine_{para}/{assm}.{hap}.peregrine_{para}.tsv"
    run:
        ...
        if str(wildcards.para) == "default":
            ...
            for i in input.fqs:
                file.write(i + "\n")
            file.close()
            shell("export PATH={peregrine_env_python}:$PATH && "
                  "echo yes|{peregrine_env_python}/python {peregrine_src} asm {input_file} 24 24 24 24 24 24 24 24 24 "
                  ""
                  " "
                  "--with-consensus --shimmer-r 3 --best_n_ovlp 8 "
                  "--output {output_file} 2>{log} 1>{log}")
        # shell("docker run  -v {dir_input}:/input -v {dir_output}:/output "
        #       "cschin/peregrine:1.6.3 asm {docker_input_file} 24 24 24 24 24 24 24 24 24 "
        #       "--with-consensus --shimmer-r 3 --best_n_ovlp 8 "
